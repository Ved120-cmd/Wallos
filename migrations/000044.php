...
$tableCheck = $db->querySingle("SELECT table_name AS name FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'uploaded_avatars'");
...
if (!$tableCheck) {
    ...
    if ($userCount === 1) {
        ...
        while ($row = $results->fetchArray(PDO::FETCH_ASSOC)) {
            $userId = $row['id'];
