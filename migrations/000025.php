...
$columnQuery = $db->query("SELECT column_name AS name FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'admin' AND column_name = 'latest_version'");
$columnRequired = $columnQuery->fetchArray(PDO::FETCH_ASSOC) === false;
...
$columnQuery = $db->query("SELECT column_name AS name FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'admin' AND column_name = 'update_notification'");
$columnRequired = $columnQuery->fetchArray(PDO::FETCH_ASSOC) === false;
