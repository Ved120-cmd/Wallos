...
if ($userIdToDelete == 1 || $userIdToDelete != $userId) {
    ...
    $logoQuery->bindValue(':id', $userIdToDelete, PDO::PARAM_INT);
    $logoResult = $logoQuery->execute();
    while ($logoResult && ($logoRow = $logoResult->fetchArray(PDO::FETCH_ASSOC))) {
    ...
    $iconQuery->bindValue(':id', $userIdToDelete, PDO::PARAM_INT);
    $iconResult = $iconQuery->execute();
    while ($iconResult && ($iconRow = $iconResult->fetchArray(PDO::FETCH_ASSOC))) {
    ...
    // Identity columns keep deleted account ids from being reused.
